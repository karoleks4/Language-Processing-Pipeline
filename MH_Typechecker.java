    ...
    static MH_TYPE computeType (MH_EXP exp, TYPE_ENV env) throws TypeError, UnknownVariable {
    	...
    	if (exp.isIF()) {
            if (computeType(exp.first(), env).equals(BoolType)) {
                ...
                } else throw new TypeError("Second and third value in the IF statement are not of the same type!");
            } else throw new TypeError("The first parameter in the IF statement is not of the type boolean!");
        }
    	...
    	if (exp.isVAR()) {
            ...
            } else throw new UnknownVariable(exp.value());
